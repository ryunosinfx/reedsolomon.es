...
class Main {
	constructor() {
		...
		const encodeFunc = () => {
			...
			const ecrRetio = ecrResioElm.value / 100;
			...
			console.log('encodeFunc presetName:' + presetName + '/ecrRetio:' + ecrRetio);
		};
		V.ael(encodeInpueElm, 'input', encodeFunc);
		...
		const decodeFunc = () => {
			...
			console.log('decodeFunc presetName:' + presetName + '/ecrRetio:' + ecrRetio);
		};
		const copyFunc = () => {
			...
			decodeInputElm.value = b642;
			decodeFunc();
		};
		V.ael(encodedCopyElm, 'click', copyFunc);
		V.ael(decodeInputElm, 'input', decodeFunc);
		const testElm = V.gid('test');
		V.ael(testElm, 'click', () => {
			this.test();
		});
	}
	test() {
		...
		console.log(u8a);
		console.log(s);
		for (const bitNum of bits) {
			const i32a = ReedSolomonES.copyToI32a(u8a, bitNum);
			const result = ReedSolomonES.copyToU8a(i32a, bitNum);
			console.log(result);
			const s = B64Util.u8aToUtf8(result);
